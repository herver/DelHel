		...
		for (auto& [wp, js] : jss.items()) {
			...
			for (auto it = jrwys.items().begin(); it != jrwys.items().end(); ++it) {
				sidinfo si{
					...
					it.value().value<std::string>("unlink_jet_dep", ""), // Unlink Departure for LFFF
					it.value().value<std::string>("prop_dep", ""), // dep
					it.value().value<std::string>("unlink_prop_dep", ""), // Unlink Departure for LFFF
...
validation CDelHel::ProcessFlightPlan(EuroScopePlugIn::CFlightPlan& fp, bool nap, bool validateOnly)
{
	...
	if (validateOnly) {
		...
		std::map<std::string, sidinfo>::iterator rwy{};
		std::string rwycfl = fpd.GetDepartureRwy();
		rwy = sid.rwys.find(rwycfl);
		sidinfo sidinfo = rwy->second;
		...
		if (cfl != sidinfo.cfl && (cfl != fp.GetFinalAltitude() || fp.GetFinalAltitude() >= sidinfo.cfl)) {
			res.valid = false;
			res.tag = "CFL";
		...
		//Début restriction Jet/Prop


		std::string airtype;
		airtype = fpd.GetEngineType();			//TYPE ENGINE

		//récupé la sid a affecté

		//Fin restriction

		sidinfo sidinfo = sit->second;

		std::ostringstream sssid;  //choix de la sid
		...
		if (nap && sidinfo.nap != "") {
			...
			if (unlink && sidinfo.unlink_prop_dep != "") {
				...
				this->LogMessage("Invalid flightplan, no matching SID found for this aircraft" + airtype, cs);
